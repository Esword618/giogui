...
func main() {
	ttfs, err := os.Open("D:\\goproject\\src\\guigio\\test\\ttfs")
	if err != nil {
		log.Fatal(err)
	}
	defer ttfs.Close()
...
func do(ttfName string) {
	...
	src, err := ioutil.ReadFile(filepath.Join("D:\\goproject\\src\\guigio\\test\\ttfs", ttfName))
	if err != nil {
		log.Fatal(err)
	}
	...
	// 	panic(err)
	// }
	if err := ioutil.WriteFile(filepath.Join(pkgName, "data.go"), b.Bytes(), 0666); err != nil {
		panic(err)
		// log.Fatal(err)
	}
}
